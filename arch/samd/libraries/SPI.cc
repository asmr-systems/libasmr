/*
 * SPI Controller library for Arduino Zero.
...
static inline SercomDataOrder getBitOrder(SPISettings& settings) {
    return (settings.getBitOrder() == BitOrder::MSBFIRST ? MSB_FIRST : LSB_FIRST);
}
...
SPIClass::SPIClass(
    ...
    _padTx(padTx),
    _padRx(padRx),
    settings(0, BitOrder::MSBFIRST, SPI_MODE0)
{}
...
void SPIClass::begin()
{
  ...
  // assign pin peripherals
  pinPeripheral(_cipo, g_PinTable[_cipo].type);
  pinPeripheral(_sck,  g_PinTable[_sck].type);
  pinPeripheral(_copi, g_PinTable[_copi].type);
